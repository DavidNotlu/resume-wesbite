...
//CANVAS 1 INTERACTABLE BACKGROUND
...
let mouse = {
    x: null,
    y: null,
    radius: canvas.height / (canvas.height / 12) * canvas.width / (canvas.width / 12)
};
...
class Particle {
    ...
    update() {
        //check is particles are within canvas
...
window.addEventListener("resize", function() {
    ...
    mouse.radius = canvas.height / (canvas.height / 12) * canvas.width / (canvas.width / 12);
    init();
});
//mouse out event
...
init();
animate(); //CANVAS 1 END

},{}]},["8N6s1","aitAD"], "aitAD", "parcelRequire7c69")
